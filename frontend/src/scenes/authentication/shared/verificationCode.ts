...
export function verificationCodeErrorMessage(e: { code?: string; detail?: string }): string {
    if (e.code === 'too_many_attempts') {
        return i18n.t('verificationCode.tooManyIncorrectAttempts', {
            defaultValue: 'Too many incorrect attempts. Request a new code and try again.',
        })
    }
    if (e.code === 'throttled') {
        return i18n.t('verificationCode.tooManyAttempts', {
            defaultValue: 'Too many attempts. Wait a few minutes and try again.',
        })
    }
    return (
        e.detail ||
        i18n.t('verificationCode.invalidOrExpired', { defaultValue: 'This code is invalid or has expired.' })
    )
}
