...
const PROJECT_SECRET_SCOPE_OBJECT_NAMES: Record<string, string> = {
    llm_gateway: i18n.t('settings.project.projectSecretAPIKeys.aiGateway', { defaultValue: 'AI gateway' }),
}
...
export const projectSecretAPIKeysLogic = kea<projectSecretAPIKeysLogicType>([
    ...
    forms(({ values, actions }) => ({
        editingKey: {
            ...
            errors: ({ label, scopes }) => ({
                label: !label
                    ? i18n.t('settings.project.projectSecretAPIKeys.needsLabel', {
                          defaultValue: 'Your API key needs a label',
                      })
                    : undefined,
                scopes: !scopes?.length
                    ? (i18n.t('settings.project.projectSecretAPIKeys.needsScope', {
                          defaultValue: 'Your API key needs at least one scope',
                      }) as any)
                    : undefined,
            }),
            submit: async (payload, breakpoint) => {
                ...
                try {
                    ...
                    if (values.editingKeyId === 'new') {
                        ...
                        lemonToast.success(
                            i18n.t('settings.project.projectSecretAPIKeys.updated', {
                                defaultValue: 'Project secret API key updated',
                            })
                        )
                    }
                    ...
                    lemonToast.error(
                        i18n.t('settings.project.projectSecretAPIKeys.saveFailed', {
                            defaultValue: 'Failed to save project secret API key',
                        })
                    )
                    throw error
                }
